...
When votes are closed, each monitor historically received an email: a
final list of votes with hash ID and timestamp, time-ordered so later
ballots sit at the bottom.

## STV (`stv_tool.py`)

Meek STV engine plus `LoadData` (load a **file**, not a directory).
v3 (`steve.vtypes.stv`) and `whatif.py` (repo root) call this module.
...
`monitoring-check.pl` checks that incoming votes are from legitimate
voters.
